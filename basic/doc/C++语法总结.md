# C++基础知识
* 在结构化程序设计方法中，程序可表示为程序=数据结构+算法; 而面向对象的程序设计方法，程序可表示为程序=对象+消息
* 面向对象程序设计方法具有抽象性、封装性、继承性和多态性等特点
* 重载不是面向对象系统中包含的要素
* 同一消息为不同的对象接受时，产生的行为是不一样的，这称为多态性
* 对象是计算机内存中的一块区域，它可以存放代码和数据
* 在面向对象的程序中，对象之间只能通过消息相互通信
* 面向对象程序设计中的消息不应该包含“如何做”的信息，只应该告诉“做什么”，“如何做”让接收对象自己决定
* 一个消息不是只能产生特定的响应效果，比如多态
* 指针的值是它所指那个对象的地址值。指针的类型是它所指向对象的类型。指针的内筒是它所指向对象的值  
* 当一个函数调用出现在函数定义之前时，必须先用函数原型对函数进行声明
* 引用
  * 建立引用时，要对它初始化
  * 引用与其代表的对象具有相同的地址
* 命名空间namespace
...
* 析构函数要在执行了delete或者程序结束时才会被调用（比如main函数return前）

### 拷贝构造函数
* 拷贝构造函数必须用引用参数，为了保证所引用的对象不被修改,通常把引用参数声明为 const参数
...
* 可以使用“对象名.静态成员”或者“类名∷ 静态成员”来访问静态数据成员
* 静态数据成员可以在静态成员函数中引用,也可以在非静态成员函数中引用
* 类的静态数据成员在类内只能声明，定义和初始化必须在类外
* const成员函数由const对象来调用，且const成员函数不能修改类成员变量且不能调用非常函数，以此来保证数据安全
...
* 构造函数调用顺序
  * 父、客（成员）、子
  * 父类列表中，先调用虚继承的构造函数，再调用非虚继承的构造函数
  * 父类列表中，同优先级的父类按照从左往右顺序调用构造函数
  * 成员子对象的构造函数调用顺序按照申明顺序而非初始化列表顺序
* 若父类无缺省构造函数，编译器会给一个
...
* 派生类的构造函数的成员初始化列表中，不能包含基类的对象成员初始化（基类的子对象要通过基类的构造函数来初始化）
